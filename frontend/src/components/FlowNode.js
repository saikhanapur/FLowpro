...
const FlowNode = ({ node, onClick, isSelected }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`rounded-2xl p-6 cursor-pointer transition-all duration-300 ease-out hover:scale-[1.01] mx-auto w-full overflow-visible ${getStatusStyles()} ${
        isSelected ? 'ring-2 ring-blue-500 ring-offset-2 scale-[1.01]' : ''
      }`}
      data-testid={`flow-node-${node.id}`}
      style={{ minHeight: 'auto', minWidth: '300px' }}
    >
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-base font-semibold mb-2 leading-tight break-words tracking-tight">{node.title}</h3>
          <p className="text-sm opacity-80 leading-relaxed mb-2 break-words whitespace-normal font-normal">{node.description}</p>
          ...
          {node.actors && node.actors.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {node.actors.map((actor, idx) => (
                <span
                  key={idx}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium break-words transition-all ${
          ...
          {node.gap && (
            <div className={`mt-3 mb-0 p-3 rounded-xl flex items-start gap-2.5 text-xs font-medium border ${
              node.status === 'trigger' || node.status === 'critical-gap'
